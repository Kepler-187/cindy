...
export const DSH_RUNTIME_PACKAGE_NAMES = [
  '@deepseek-ai/dsh', '@deepseek-ai/dsh-base', '@deepseek-ai/dsh-web-app',
  '@deepseek-ai/dsh-app-boot', '@deepseek-ai/dsh-home-paths', '@deepseek-ai/dsh-agent',
...
const PACKAGES = DSH_RUNTIME_PACKAGE_NAMES.map((name) => `${name}@${DSH_VERSION}`);
export const DSH_RUNTIME_FINGERPRINT = createHash('sha256')
  .update(JSON.stringify({ schema: RUNTIME_SCHEMA_VERSION, packages: PACKAGES }))
  .digest('hex');
...
if [ -f "$ROOT/.cindy-dsh-runtime" ] && [ "$(cat "$ROOT/.cindy-dsh-runtime")" = "$2" ]; then
  cd "$ROOT"
  "$NODE" -e "for (const name of JSON.parse(process.argv[1])) require.resolve(name + '/package.json')" "$3" >/dev/null 2>&1 && exit 0
fi
cd "$ROOT"
PATH="$NODE_ROOT/bin:$PATH" "$NPM" install --ignore-scripts --omit=dev --no-audit --no-fund --package-lock=true
"$NODE" -e "for (const name of JSON.parse(process.argv[1])) require.resolve(name + '/package.json')" "$3" >/dev/null
printf '%s' "$2" > "$ROOT/.cindy-dsh-runtime"`;
  const result = await host.exec(
    `bash -c ${shellQuote(script)} -- ${shellQuote(Buffer.from(manifest).toString('base64'))} ${shellQuote(DSH_RUNTIME_FINGERPRINT)} ${shellQuote(JSON.stringify(DSH_RUNTIME_PACKAGE_NAMES))}`,
